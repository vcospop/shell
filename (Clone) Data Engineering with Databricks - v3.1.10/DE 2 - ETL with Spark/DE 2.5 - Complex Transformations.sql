...
select value:geo.city from events_strings
...
select *,explode(items) from parsed_events
...
select event_name from exploded_events

-- COMMAND ----------

select user_id, collect_set(event_name) , flatten(collect_set(items.item_id)) from exploded_events group by user_id
